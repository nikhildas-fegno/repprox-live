    ...
    <section className="relative overflow-hidden border-b border-ink/10 bg-paper pt-16 pb-24 lg:pt-20 lg:pb-32">
      ...
      <Container className="relative flex flex-col items-center text-center z-10">
        ...
        <motion.div
          ...
          className="mb-6"
        >
          <Badge variant="accent" className="border-accent-ink/20 bg-accent-soft text-accent-ink font-mono text-[11px] py-1 px-3.5 tracking-wider">
            <Sparkles className="size-3.5 text-accent-ink" />
            AI-Powered Field Force Automation v5.2
          </Badge>
        </motion.div>
        ...
        <motion.p
          ...
          className="mt-4 font-mono text-[13px] uppercase tracking-[0.25em] text-[#ff9800] font-bold"
        >
        ...
        >
          RepPro X helps distributors, wholesalers, FMCG companies, pharmaceutical suppliers, and field sales organizations automate sales, collections, inventory management, route execution, customer engagement, and field productivity from a single unified platform.
        </motion.p>
        ...
        >
          {benefits.map((benefit) => (
            <div key={benefit} className="flex items-center gap-2 text-sm text-ink-soft font-semibold">
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-white/5 bg-slate-900/50 px-4 py-3 rounded-t-lg">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-4 font-mono text-[9px] text-slate-500 tracking-widest uppercase">RepPro X Management Portal v5.2</span>
            </div>
          ...
          <div className="bg-[#0b0e17] p-4 lg:p-6 rounded-b-lg grid gap-4 lg:gap-6 text-left">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-slate-900/60 border border-white/5 p-4 rounded-lg flex flex-col gap-1 relative overflow-hidden">
                <div className="absolute top-3 right-3 text-slate-500"><DollarSign className="size-4 text-[#0ea5ff]" /></div>
                <span className="text-[11px] font-mono text-slate-400 uppercase tracking-wider">Daily Sales Volume</span>
                <span className="text-xl lg:text-2xl font-semibold text-white tracking-tight">$142,840</span>
                <span className="text-[11px] text-emerald-400 flex items-center gap-1 font-medium mt-1">
                  <TrendingUp className="size-3" /> +12.4% <span className="text-slate-500">vs yesterday</span>
                </span>
              </div>
              
              {/* Cash Collections */}
              <div className="bg-slate-900/60 border border-white/5 p-4 rounded-lg flex flex-col gap-1 relative overflow-hidden">
                <div className="absolute top-3 right-3 text-slate-500"><Activity className="size-4 text-[#ff9800]" /></div>
                <span className="text-[11px] font-mono text-slate-400 uppercase tracking-wider">Collections Cash/Cheque</span>
                <span className="text-xl lg:text-2xl font-semibold text-white tracking-tight">$86,430</span>
                <div className="flex items-center justify-between mt-2">
                  <span className="text-[11px] text-slate-400">Target Achieved</span>
                  <span className="text-[11px] font-mono font-semibold text-[#ff9800]">94.2%</span>
                </div>
                <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden mt-1">
                  <div className="bg-[#ff9800] h-full rounded-full" style={{ width: "94%" }} />
                </div>
              </div>

              {/* Route Compliance */}
              <div className="bg-slate-900/60 border border-white/5 p-4 rounded-lg flex flex-col gap-1 relative overflow-hidden">
                <div className="absolute top-3 right-3 text-slate-500"><MapPin className="size-4 text-emerald-400" /></div>
                <span className="text-[11px] font-mono text-slate-400 uppercase tracking-wider">Route Compliance</span>
                <span className="text-xl lg:text-2xl font-semibold text-white tracking-tight">97.8%</span>
                <span className="text-[11px] text-slate-400 mt-2 flex items-center gap-1 font-medium">
                  <span className="size-2 rounded-full bg-emerald-500" /> 42/43 Vans GPS Verified
                </span>
              </div>

              {/* AI Smart Ordering Counter */}
              <div className="bg-slate-900/60 border border-white/5 p-4 rounded-lg flex flex-col gap-1 relative overflow-hidden">
                <div className="absolute top-3 right-3 text-slate-500"><Sparkles className="size-4 text-cyan-400" /></div>
                <span className="text-[11px] font-mono text-slate-400 uppercase tracking-wider">AI Ordering Orders</span>
                <span className="text-xl lg:text-2xl font-semibold text-white tracking-tight">348 Recs</span>
              ...
              <div className="lg:col-span-8 bg-slate-900/40 border border-white/5 p-5 rounded-lg flex flex-col justify-between">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex flex-col">
                    <span className="text-[13px] font-mono text-slate-400 uppercase tracking-wider">Route Operations & Sales Performance</span>
                    <span className="text-sm font-semibold text-slate-300">Weekly Target Progress</span>
                  </div>
                  <div className="flex gap-3 text-[10px] font-mono">
                    <span className="flex items-center gap-1.5 text-slate-400">
                      <span className="size-2.5 rounded bg-[#1d4ed8]" /> Actual Sales
                    </span>
                    <span className="flex items-center gap-1.5 text-[#ff9800]">
                      <span className="size-2.5 rounded bg-[#ff9800]" /> Target Route
                    </span>
                  </div>
                </div>
                ...
                <div className="h-56 w-full relative">
                  <svg className="w-full h-full" viewBox="0 0 600 220" preserveAspectRatio="none">
                    ...
                    <line x1="0" y1="170" x2="600" y2="170" stroke="rgba(255,255,255,0.04)" strokeWidth="1" />

                    {/* Gradient Fill under Path */}
                    <defs>
                      <linearGradient id="chartGlow" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="0%" stopColor="#0ea5ff" stopOpacity="0.25" />
                        <stop offset="100%" stopColor="#0ea5ff" stopOpacity="0.0" />
                      </linearGradient>
                    </defs>
                    ...
                    <motion.path
                      ...
                      transition={{ duration: reduceMotion ? 0 : 1.5, delay: 0.8, ease: "easeOut" }}
                    />
                  </svg>
                  ...
                  <div className="absolute top-12 left-1/3 bg-slate-950/90 border border-white/10 px-3 py-1.5 rounded shadow-xl pointer-events-none text-xs flex flex-col gap-0.5">
                    <span className="text-[10px] font-mono text-slate-500">Route Region West</span>
                    <span className="font-semibold text-slate-200">$48,210 (Visit Done)</span>
                  </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-4 bg-slate-900/40 border border-white/5 p-5 rounded-lg flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[10px] font-mono text-emerald-400 uppercase tracking-widest bg-emerald-500/10 border border-emerald-500/20 py-0.5 px-2 rounded-full">Active</span>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="bg-slate-950/50 border border-white/5 p-3 rounded flex flex-col gap-1">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-xs text-white">Solace FMCG Retail</span>
                        <span className="text-[9px] font-mono text-cyan-400 bg-cyan-500/10 px-1.5 py-0.5 rounded">94% match</span>
                      </div>
                      <span className="text-[10px] text-slate-400 font-mono">Recommend replenishment: +12 cases Soda</span>
                      <span className="text-[10px] text-[#ff9800]">Upsell opportunity: 4 cases Diet Drink</span>
                    </div>

                    {/* Item 2 */}
                    <div className="bg-slate-950/50 border border-white/5 p-3 rounded flex flex-col gap-1">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-xs text-white">MedPlus Pharmacies</span>
                        <span className="text-[9px] font-mono text-cyan-400 bg-cyan-500/10 px-1.5 py-0.5 rounded">87% match</span>
                      </div>
                      <span className="text-[10px] text-slate-400 font-mono">Stock level depletion alert: Vitamin C pack</span>
                      <span className="text-[10px] text-emerald-400">Order suggested: 25 cases (FIFO rule)</span>
                    </div>

                    {/* Item 3 */}
                    <div className="bg-slate-950/50 border border-white/5 p-3 rounded flex flex-col gap-1">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-xs text-white">Eastside Wholesalers</span>
                        <span className="text-[9px] font-mono text-cyan-400 bg-cyan-500/10 px-1.5 py-0.5 rounded">91% match</span>
                      </div>
                      <span className="text-[10px] text-slate-400 font-mono">Promotional alert: Bulk Material bundle</span>
                      <span className="text-[10px] text-slate-500">Suggested discount: 15% applied</span>
                    </div>
                  </div>
                </div>

                <div className="mt-4 pt-3 border-t border-white/5 text-center">
                  <Link href="/features#ai-ordering" className="text-[11px] font-mono text-slate-400 hover:text-white uppercase tracking-wider inline-flex items-center gap-1">
